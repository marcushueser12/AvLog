...
router.post('/save-scan', async (req: AuthRequest, res) => {
  try {
    ...
    const dbEntries = entries.map((entry: LogbookEntry) => ({
      ...
      mel: entry.mel?.trim() || null,
      seaplane_time: entry.seaplaneTime?.trim() || null,
      simulator_time: entry.simulatorTime?.trim() || null,
...
router.put('/update-scan', async (req: AuthRequest, res) => {
  try {
    ...
    const dbEntries = entries.map((entry: LogbookEntry) => ({
      ...
      mel: entry.mel?.trim() || null,
      seaplane_time: sanitizeString(entry.seaplaneTime, 50) || null,
      simulator_time: sanitizeString(entry.simulatorTime, 50) || null,
...
router.get(
  ...
  try {
    ...
    const transformedEntries = sortedEntries.map((entry: any) => ({
      ...
      mel: entry.mel || '',
      seaplaneTime: entry.seaplane_time || '',
      simulatorTime: entry.simulator_time || '',
